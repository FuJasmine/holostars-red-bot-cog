...
class StarStream(commands.Cog):
    ...
    async def _channel_set(self, ctx: commands.Context, channel_name_or_id: str, mention_channel: discord.TextChannel, chat_channel: discord.TextChannel):
        """Set tracking YouTube channel.
        
        Use: [p]stars channel set [YT channel id | YT channel name] [mention channel id] [default chat channel id]
        """
        ...
        if not stream:
            ...
            if not self.check_name_or_id(channel_name_or_id):
                stream = YouTubeStream(
                    ...
                    , config=self.config, chat_channel_id=chat_channel.id
                    , mention_channel_id=mention_channel.id
                )
            else:
                stream = YouTubeStream(
            ...
            stream.chat_channel_id = chat_channel.id         
            stream.mention_channel_id = mention_channel.id  
            self.streams.append(stream)
            await ctx.send(
                _(
